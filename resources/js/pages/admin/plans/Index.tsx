...
export default function PlanIndex({ plans, flash }: Props) {
    ...
    const handleStatusToggle = (id: number, currentStatus: boolean) => {
        router.get(
            route('admin.plans.toggleStatus', id),
            {},
            {
                preserveState: true,
                onSuccess: () => {
                    setData((prev) => prev.map((plan) => (plan.id === id ? { ...plan, status: !currentStatus } : plan)));
    ...
    const columns: ColumnDef<Plan>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Button
                    variant="outline"
                    className={`${row.original.status ? 'bg-green-800' : 'bg-red-600'} text-white hover:text-white`}
