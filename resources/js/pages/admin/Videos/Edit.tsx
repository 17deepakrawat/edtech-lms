...
export default function Edit({ video, courses }: Props) {
  const [units, setUnits] = useState<Unit[]>([]);
  const [topics, setTopics] = useState<Topic[]>([]);
  const [preview, setPreview] = useState<string | null>(video.video_path);
  const [isCalculatingDuration, setIsCalculatingDuration] = useState(false);

  const { data, setData, post, errors, reset } = useForm({
    course_id: video.course_id?.toString() || '',
    unit_id: video.unit_id?.toString() || '',
    topic_id: video.topic_id?.toString() || '',
    name: video.name,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(route('videos.update', video.id), formData, {
      forceFormData: true,
      preserveScroll: true,
      onSuccess: () => {
      ...
      onError: (errors) => {
        toast.error(Object.values(errors)[0] || 'Failed to update video');
      },
    });
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {data.video_type === 'local' && (
              <div>
                ...
                {preview && (
                  <div className="mt-2">
                    <video src={preview} controls className="max-w-md" />
                  </div>
                )}
            ...
            {data.video_type === 'embed' && (
              <div>
                ...
                <Input value={data.embed_url} onChange={e => setData('embed_url', e.target.value)} />
            ...
            <div className="flex gap-4">
              ...
              <Button type="button" variant="outline" onClick={() => router.visit(route('videos.index'))}>
                Cancel
              </Button>
            </div>
          </form>
