...
export default function Edit({ blog, blogCategories }: EditProps) {
    ...
    const parsedFaq: FaqItem[] = Array.isArray(blog.faq) ? blog.faq : JSON.parse(blog.faq || '[]');
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                {errors.blog_category_id && <p className="mt-1 text-sm text-red-500">{errors.blog_category_id}</p>}
                            </div>

                            <div>
                                <Label>Blog Title</Label>
                                <Input value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="Enter blog title" />
                        ...
                        /> */}
                            <RichTextEditor
                                value={data.short_description}
                                onChange={(short_description) => setData('short_description', short_description)}
                            />
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                />
                                {errors.author_name && <p className="mt-1 text-sm text-red-500">{errors.author_name}</p>}
                            </div>

                            <div>
                                <Label>Change Author Image</Label>
                                <Input type="file" accept="image/*" onChange={(e) => setData('author_image', e.target.files?.[0] ?? null)} />
                                {errors.author_image && <p className="mt-1 text-sm text-red-500">{errors.author_image}</p>}
                                {blog.author_image && (
                                    <img src={`/storage/${blog.author_image}`} className="mt-2 h-32 w-32 rounded-lg object-cover" alt="Author" />
                                )}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {blog.image && <img src={`/storage/${blog.image}`} className="mt-2 h-48 w-full rounded-lg object-cover" alt="Blog" />}
                        </div>
                        ...
                        <div className="mt-8">
                            ...
                            {errors.faq && <p className="mt-2 text-sm text-red-500">{errors.faq}</p>}
                        </div>
